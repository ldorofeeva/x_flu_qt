...
class GUI(QDialog):
    ...
    def create_erange_grp(self):
        ...
        self.e_from.textChanged[str].connect(self.type_e_from)
        self.e_to_lbl = QLabel('to: ')
        self.e_to = QLineEdit(self)
        self.e_to.setText(str(self.erange_slider.high()))
        self.e_to.textChanged[str].connect(self.type_e_to)
    ...
    def open_file(self):
        ...
        fname_val, _ = QFileDialog.getOpenFileName(self,"QFileDialog.getOpenFileName()", "","All Files (*);;Python Files (*.py)", options=options)
        if fname_val:
    ...
    def load_entry(self, ename_val):
        if ename_val:
            ...
            with h5py.File(fname_val, "r") as h5f:
                xs = h5f[ename_val + '/' + self.Nexus_spec['subgrp'] + '/' + self.Nexus_spec['ds_x']][:]
                self.ys = h5f[ename_val + '/' + self.Nexus_spec['subgrp'] + '/' + self.Nexus_spec['ds_y']][:]
                self.signal = h5f[ename_val + '/' + self.Nexus_spec['subgrp'] + '/' + self.Nexus_spec['ds_signal']][:]
    ...
    def type_e_from(self, e_from):
        if e_from != str(self.erange_slider.low()):
            self.erange_slider.setLow(int(e_from))
            self.update_plot()
            self.update_lines()
        
    def type_e_to(self, e_to):
        if e_to != str(self.erange_slider.high()):
            self.erange_slider.setHigh(int(e_to))
            self.update_plot()
            self.update_lines()
    ...
    def update_lines(self):
        ...
        if e_to_val >= e_from_val:
            matches = self.xrelines_names[np.where(np.logical_and(self.xrelines_vals>=e_from_val, self.xrelines_vals<=e_to_val))[0]]
            s =  '\n'.join(el + ' (' + ', '.join(l for l in matches[np.where(matches[:,0]==el)][:,1])+')' for el in np.unique(matches[:,0]))
